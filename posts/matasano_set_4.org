...
AES-CBS (Cipher Block Chaining) where we scrambled the plaintext with
...
cryptanalyst can manipulate the cipher text.  This provides two attack
vectors, the first allowing us to edit the underlying plaintext. The
second allows us to recover the plaintext if the validity of the
padding is leaked to us. An additional exploit is possible if the key
is re-used as the IV (Initialization Vector) of CBC.  Since the
cryptanalyst controls the first block of cipher text, it is possible
to extract the IV (and subsequently the key) allowing complete
