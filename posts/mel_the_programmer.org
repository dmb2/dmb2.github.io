...
does what I want". A well as "if it ain't broke don't fix it," there
is a framework for how to approach development.  The idea is to
approach code the way you approach literature. Your first draft is
getting your ideas on the page. Editing from the first draft replaces
"once it works I'm done." You would never publish the first draft of
anything.  In the same way that you should never publish the first
...
junction to work. In Mel's case he would be able to take advantage of
the system's quirks in the compiler.  Everyone would benefit from
Mel's low level understanding of the system.
